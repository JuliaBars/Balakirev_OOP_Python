"""
Подвиг 11 (на повторение). Объявите класс для мессенджера с именем Viber. В
этом классе должны быть следующие методы:
...
class Viber:
    ...
    def show_last_message(cls, number):
        if number <= len(cls.msgs):
            for mess in tuple(cls.msgs.values())[-number:]:
                print(mess.text)
        else:
            for mess in tuple(cls.msgs.values()):
                print(mess.text)
