...
для класса MotherBoard: name - наименование; cpu - ссылка на объект класса
CPU; total_mem_slots = 4 - общее число слотов памяти (атрибут прописывается
с этим значением и не меняется); mem_slots - список из объектов класса Memory
...
class MotherBoard:
    ...
    def __init__(self, name, cpu, *args) -> None:
        self.name = name
        self.cpu = cpu
    ...
    def get_config(self):
        return [
            ...
            'Память: ' + '; '.join(
                map(lambda x: f'{x.name} - {x.volume}', self.mem_slots)
            )
        ]
